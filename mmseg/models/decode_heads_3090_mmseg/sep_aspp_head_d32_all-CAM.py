...
class DepthwiseSeparableASPPHead(ASPPHead):
    ...
    def __init__(self, c1_in_channels, c1_channels, **kwargs):
        ...
        self.aspp_modules = DepthwiseSeparableASPPModule(
            ...
            norm_cfg=self.norm_cfg,
            act_cfg=self.act_cfg)
        self.c4_cam_out_conv = ConvModule(
            self.c4_channels,
            self.channels,
            1,
            # padding=1,
            ...
            norm_cfg=self.norm_cfg,
            act_cfg=self.act_cfg)
        self.c3_cam_out_conv = ConvModule(
            self.c3_channels,
            self.channels,
            1,
            # padding=1,
    ...
    def forward(self, inputs):
        ...
        # c4跨层
        # c4_output = self.c4_cam_in_conv(inputs[3])
        c4_output = self.c4_CAM(inputs[3])
        c4_output = self.c4_cam_out_conv(c4_output)
        # print("c4_output: ".format(c4_output.shape))
        output = torch.cat([output, c4_output], dim=1)      # channels = 512+2048
        output = self.bottleneck4(output)       # 3x3conv channels = 512
        # print("bottleneck4: {}".format(output.shape))
        # c3跨层 2倍上采样
        # c3_output = self.c3_cam_in_conv(inputs[2])
        c3_output = self.c3_CAM(inputs[2])
        c3_output = self.c3_cam_out_conv(c3_output)
        output = resize(
            input=output,
        ...
        c2_output = self.c2_CAM(inputs[1])
        output = resize(
            input=output,
        ...
        if self.c1_CAM is not None:
            # c1_output = self.c1_pam_in_conv(inputs[0])
            c1_output = self.c1_CAM(inputs[0])
            output = resize(
                input=output,
                size=inputs[0].shape[2:],
                mode='bilinear',
